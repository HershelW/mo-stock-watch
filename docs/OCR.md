...
OCR 不会直接覆盖原持仓。识别结果会进入差异预览，只有人工确认后才写入目标账户。
...
5. 要求模型返回持仓和可识别的账户汇总结构化 JSON。
6. 区分东方财富的“现价/成本”和财通证券的“成本/现价”列顺序。
7. 只保留 6 位代码、数量大于 0、成本价大于 0 的结果。

API Key 由当前 Windows 用户加密保存在 `credentials.dat`，`settings.json` 中不会保留明文。
